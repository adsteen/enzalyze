...
#     d2 <- read.csv(file = file2$datapath, sep = input$sep2, header = input$header2,
#                    stringsAsFactors = input$stringAsFactors2)
#     dr_uncal <- enzalyze_reform(d = d2, .labels = input$substrate, the.date = input$date)
#   })
#   
#   # Create data plot, faceted by substrate
#   output$p_data <- renderPlot({
#     print(data_plotr(data = df_uncal(), datalabel = "Raw Data site ", 
...
#     print(curve_plotr(data = d_cal(), concentration.variable = input$concentration.variable, 
...
#     print(v0plotr(data = lm_dframe(), v0label = "Calibrated v0 site ", 
...
#     tabsetPanel(tabPanel("Data", plotOutput("p_data")),
