...
#' @param .data - data frame to be fed to plyr and lm_stats functions
...
uncalib_slope <- function(.data, id.var, xvar="elapsed", yvar="RFU"){
  
  # split a data frame by the specified id-variables and apply lm_stats, then return the result in a data frame
  lm_dframe <- ddply(.data, .variables = id.var, function(x) lm_stats(x, xvar, yvar))
